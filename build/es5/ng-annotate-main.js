...
function namespaceArray(ctx, functionExpression) {
    function getNamespaceParamName(ctx, identifierNode) {
        ...
        for (var i = 0; i < comments.length; i++) {
            ...
            var match = comment.value.match(/(?:^|\s*)@(?:namespace|ns)\s*([^\s]*)/);
            var namespace = (match && match[1] ? match[1] : null);
            if (!namespace || namespace.length == 0) {
                continue;
            }

            var commentNode = ctx.lut.findNodeFromPos(comment.range[1]);
            if (!commentNode) {
                continue;
            }
    ...
    for (var i = 0; i < functionExpression.params.length; i++) {
...
module.exports = function ngAnnotate(src, options) {
    ...
    var nsSeparator = options.namespace_separator || '.';
